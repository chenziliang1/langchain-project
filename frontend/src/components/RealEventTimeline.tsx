import { CalendarDays, TrendingUp, TrendingDown, Minus, ExternalLink } from 'lucide-react';
import type { ActorActivityItem } from '../types';

interface Props {
  timeline: ActorActivityItem[];
}
...
export default function RealEventTimeline({ timeline }: Props) {
  if (!timeline || timeline.length === 0) return null;

  const maxArticles = Math.max(...timeline.map(t => t.total_articles || 0), 1);
    ...
    <div className="panel" style={{ background: '#fff', border: '1px solid #e2e8f0' }}>
      <h3 style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, fontSize: 15, fontWeight: 700, color: '#1a1a1a' }}>
        ...
        <span style={{ fontSize: 11, color: '#888', fontWeight: 400, marginLeft: 8 }}>
          Event Database daily aggregation
        </span>
      </h3>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #e5e7eb', textAlign: 'left' }}>
              ...
              <th style={{ padding: '8px 6px', color: '#6b7280', fontWeight: 600 }}>Severe Events</th>
            </tr>
          </thead>
          <tbody>
            {timeline.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: '8px 6px', maxWidth: 140 }}>
                    <span style={{
                      display: 'inline-block',
                      padding: '2px 8px',
                      borderRadius: 6,
                  ...
                  <td style={{ padding: '8px 6px' }}>
                    <span style={{ fontSize: 11, color: '#9ca3af' }}>
                      {item.severe_conflict || 0} conflict / {item.severe_cooperation || 0} coop
                    </span>
                  </td>
                </tr>
              );
      ...
      <div style={{ marginTop: 12, fontSize: 11, color: '#9ca3af', display: 'flex', gap: 16 }}>
        <span>Total days: {timeline.length}</span>
        <span>Total events: {timeline.reduce((sum, t) => sum + (t.total_events || 0), 0).toLocaleString()}</span>
        <span>Total articles: {timeline.reduce((sum, t) => sum + (t.total_articles || 0), 0).toLocaleString()}</span>
      </div>
    </div>
  );
}
